#!/usr/bin/env python3
"""Run the predeclared qualifying-second-node D R3 sensitivity once."""
...
WALK_METADATA = ROOT / "Data/metadata/transport/transport_d_walk_pairs_r3.json"
WALK_AUDIT = ROOT / "Results/pilots/transport_d_walk_pair_r3_audit.json"
ORIGIN_WALK = ROOT / "Data/interim/transport/d_pedestrian_origin_station_walk_r3.parquet"
DESTINATION_WALK = ROOT / "Data/interim/transport/d_pedestrian_station_destination_walk_r3.parquet"
MARKER = ROOT / "Results/transport_d_r3_sensitivity.json"
CELL_OUT = ROOT / "Data/processed/transport/d_project_attribution_cell_r3.parquet"
CITY_OUT = ROOT / "Results/tables/transport_d_project_city_summary_r3.parquet"
SUMMARY_OUT = ROOT / "Results/tables/transport_d_project_summary_r3.csv"
...
def validate() -> tuple[dict, dict, dict]:
    if MARKER.exists():
        raise RuntimeError("R3 sensitivity marker exists; rerun prohibited")
    ...
    walk_audit = json.loads(WALK_AUDIT.read_text(encoding="utf-8"))
    if not walk_audit["passed"]:
        raise RuntimeError("R3 walk-pair audit failed")
    walk_meta = json.loads(WALK_METADATA.read_text(encoding="utf-8"))
    for relative_path, item in walk_meta["outputs"].items():
        if sha256(ROOT / relative_path) != item["sha256"]:
            raise RuntimeError(f"R3 walk-pair output changed: {relative_path}")
    return r0, preopen, walk_meta


def main() -> int:
    r0, preopen, walk_meta = validate()
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    base_cfg = json.loads((ROOT / cfg["base_run"]).read_text(encoding="utf-8"))
    run_cfg = {**base_cfg, **cfg, "impedance_variant": "r0"}
    ...
    contract = json.loads((ROOT / base_cfg["contract"]).read_text(encoding="utf-8"))
    city_order = {
    ...
    for year in range(2018, 2025):
        ...
        values = np.empty((1 << len(packages), len(origins), len(metrics)), dtype=np.float64)
        ...
        if not closure["passed"]:
            raise RuntimeError(f"R3 Shapley closure failed for {year}")
    ...
    result = {
        ...
        "walk_metadata_sha256": sha256(WALK_METADATA),
        "replacement_counts": walk_meta["replacement_counts"],
